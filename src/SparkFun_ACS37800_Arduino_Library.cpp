...
//According to the MEMORY MAP in the datasheet, the bit fields for EEPROM (0B) and Shadow (1B) are different. Another typo...!?
ACS37800ERR ACS37800::setCurrentCoarseGain(ACS37800_CRS_SNS_e gain, boolean _eeprom)
{
  ...
  if (error != ACS37800_SUCCESS)
  {
    if (_printDebug == true)
    {
      _debugPort->print(F("setCurrentCoarseGain: writeRegister (2F) returned: "));
      _debugPort->println(error);
    }
    return (error); // Bail
  }
  ...
  error = readRegister(&store.data.all, ACS37800_REGISTER_SHADOW_1B); // Read register 1B
  ...
  float existingGain = ACS37800_CRS_SNS_GAINS[store.data.bits.crs_sns];
  if (_printDebug == true)
  {
    _debugPort->print(F("setCurrentCoarseGain: shadow gain setting is currently: "));
    _debugPort->println(existingGain, 1);
  }
  ...
  _currentCoarseGain = ACS37800_CRS_SNS_GAINS[gain]; // Update the copy used by the current calculation

  if (_eeprom) // Check if user wants to set eeprom too
  {
    error = readRegister(&store.data.all, ACS37800_REGISTER_EEPROM_0B); // Read register 0B
    ...
    existingGain = ACS37800_CRS_SNS_GAINS[store.data.bits.crs_sns];
    if (_printDebug == true)
    {
      _debugPort->print(F("setCurrentCoarseGain: eeprom gain setting is currently: "));
      _debugPort->println(existingGain, 1);
    }

    store.data.bits.crs_sns = (uint32_t)gain; //Adjust the gain

    error = writeRegister(store.data.all, ACS37800_REGISTER_EEPROM_0B); // Write register 0B
...
ACS37800ERR ACS37800::setIavgSelEn(boolean iavgselen, boolean _eeprom)
{
  ACS37800ERR error = writeRegister(ACS37800_CUSTOMER_ACCESS_CODE, ACS37800_REGISTER_VOLATILE_2F); // Set the customer access code
  ...
  if (error != ACS37800_SUCCESS)
  {
    if (_printDebug == true)
    {
      _debugPort->print(F("setIavgSelEn: writeRegister (2F) returned: "));
      _debugPort->println(error);
    }
    return (error); // Bail
  }
  ...
  if (error != ACS37800_SUCCESS)
  {
    if (_printDebug == true)
    {
      _debugPort->print(F("setIavgSelEn: readRegister (1B) returned: "));
      _debugPort->println(error);
    }
    return (error); // Bail
  }
  ...
  if (_printDebug == true)
  {
    _debugPort->print(F("setIavgSelEn: iavgselen is currently: "));
    _debugPort->println(store.data.bits.iavgselen);
  }

  if (iavgselen) //Adjust iavgselen
  {
  ...
  if (error != ACS37800_SUCCESS)
  {
    if (_printDebug == true)
    {
      _debugPort->print(F("setIavgSelEn: writeRegister (1B) returned: "));
      _debugPort->println(error);
    }
    return (error); // Bail
  }
  ...
  if (_eeprom) // Check if user wants to set eeprom too
  {
    ...
    if (error != ACS37800_SUCCESS)
    {
      if (_printDebug == true)
      {
        _debugPort->print(F("setIavgSelEn: readRegister (0B) returned: "));
        _debugPort->println(error);
      }
      return (error); // Bail
    }
    ...
    if (_printDebug == true)
    {
      _debugPort->print(F("setIavgSelEn: eeprom iavgselen is currently: "));
      _debugPort->println(store.data.bits.iavgselen);
    }
    ...
    if (error != ACS37800_SUCCESS)
    {
      if (_printDebug == true)
      {
        _debugPort->print(F("setIavgSelEn: writeRegister (0B) returned: "));
        _debugPort->println(error);
      }
    }
  }
...
//// Read and return the iavgselen flag from shadow memory
ACS37800ERR ACS37800::getIavgSelEn(boolean *iavgselen)
{
  ACS37800_REGISTER_0B_t store;
  ACS37800ERR error = readRegister(&store.data.all, ACS37800_REGISTER_SHADOW_1B); // Read register 1B
  ...
  if (error != ACS37800_SUCCESS)
  {
    if (_printDebug == true)
    {
      _debugPort->print(F("getIavgSelEn: readRegister (1B) returned: "));
      _debugPort->println(error);
    }
    return (error); // Bail
  }
  ...
  if (_printDebug == true)
  {
    _debugPort->print(F("getIavgSelEn: iavgselen is currently: "));
    _debugPort->println(store.data.bits.iavgselen);
  }

  *iavgselen = (boolean)store.data.bits.iavgselen; //Return iavgselen
